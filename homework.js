let people = [{"id":1,"firstName":"Osmond","surName":"Kieran","sex":"Male","age":26,"isVegetarian":true,"langCount":8},
...
for (let i = 0; i < people.length; i++) {
    // peopleLocal=JSON.parse(localStorage.getItem(i));
     if (people[i].langCount>maxLang){
         maxLang=people[i].langCount;
     }
     midAge+=people[i].age;
...
midAge=midAge/people.length;
document.getElementById("age").innerHTML =midAge;
document.getElementById("lang").innerHTML =maxLang;
document.getElementById("boys").innerHTML =boys;
document.getElementById("girls").innerHTML =girls;


let eu='Австрия, Бельгия, Болгария, Великобритания, Венгрия, Германия,' +
    ' Греция, Дания, Ирландия, Испания, Италия, Кипр, Латвия, Литва, ' +
    'Люксембург, Мальта, Нидерланды, Польша, Португалия, Румыния, ' +
    'Словакия, Словения, Финляндия, Франция, Хорватия, Чехия, Швеция, Эстония'

function validateForm() {
    let  inputString = document.forms["myForm"]["fname"].value
        ,char        = inputString.charAt(0);
    ...
    if (inputString.length<4) {
        ...
        alert("Название необходимо начинать с большой буквы");
        return false;
    }
    else if(eu.includes(inputString)){
        alert('Страна входит в состав ЕС');
        return true;
    }
    else alert('Страна не входит в состав ЕС'); return false;
}